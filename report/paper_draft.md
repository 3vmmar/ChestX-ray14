...
Images are accessed from 12 NIH batch downloads placed in `data/archive/images_001/` through `data/archive/images_012/`. A master registry (`data/metadata/master_registry.csv`) maps every image to its patient ID, class labels, and view position (PA/AP). Clean binary splits are generated by `scripts/rebuild_clean_splits.py`.
